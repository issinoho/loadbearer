...
The man page (`loadbearer.1`) and shell completions are **generated by
`build.rs`** from `src/cli.rs` and bundled into the archives; they are
not committed to the repo.

Version bumps follow [`VERSIONING.md`](VERSIONING.md): a breaking change
to the CLI or a `schema`-tagged JSON format is a **major** bump; a new
subcommand or capability is a **minor** bump; a fix or UX refinement is a
**patch**. Recalibrating the baseline or raising the MSRV is a minor,
not a major — the baseline and absolute scores are explicitly outside the
stability contract.
